...
**Security Cleanup:**
- [ ] Remove temporary development access from RDS security group
  - Run: `aws ec2 describe-security-groups --group-ids sg-04dd0c32191ab8ef3 --region us-east-1 --query 'SecurityGroups[0].IpPermissions'`
  - Identify any temporary local IP access rules (Description contains "Temporary" or "local development")
  - Prompt: "Remove temporary local IP access from RDS security group? (y/n)"
  - If yes, run: `aws ec2 revoke-security-group-ingress --group-id sg-04dd0c32191ab8ef3 --security-group-rule-ids sgr-0f73cf363a3702653 --region us-east-1`
  - Verify only ECS security group (sg-08999701ba2f7d8b1) has access to RDS port 5432
- [ ] Disable public accessibility on RDS (revert development setting)
  - Run: `aws rds modify-db-instance --db-instance-identifier plccoach-db --no-publicly-accessible --region us-east-1 --apply-immediately`
  - This ensures the database is only accessible from within the VPC in production
  - Wait 2-5 minutes for modification to complete
  - Verify: `aws rds describe-db-instances --db-instance-identifier plccoach-db --region us-east-1 --query 'DBInstances[0].PubliclyAccessible'` returns `false`
